...
#### config ####
input_img_path = "./data/input_images/test.png"
output_img_path = "./data/output_images/detected_fridge.jpg"
...
def draw_bbox(image, bboxes, classes=read_class_names(class_name_path), show_label=True):
    """
...
def process_obj_classify(detections, original_image_size):
...
def indicate_item_name(bboxes, classes=read_class_names(class_name_path)):
    """
    bboxes: [x_min, y_min, x_max, y_max, probability, cls_id] format coordinates.
    """
    item_set = set()
    for i, bbox in enumerate(bboxes):
        class_ind = int(bbox[5])
        item_set.add(classes[class_ind])
...
def wimp_it(return_img = False):
    ...
    if return_img:
        ...
        return item_set, cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
